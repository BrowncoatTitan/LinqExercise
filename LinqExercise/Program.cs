...
namespace LinqExercise
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var sevenPlus = numbers.Where(x => x > 6);
            foreach (var x in sevenPlus) Console.WriteLine(x);
            Console.WriteLine("------------------------------------");
            //TODO: Order numbers in any order (ascending or desc) but only print 4 of them **foreach loop only!**
            var onlyFour = numbers.OrderBy(x => x).Take(4);
            foreach (var x in onlyFour) Console.WriteLine(x);
            Console.WriteLine("------------------------------------");
            //TODO: Change the value at index 4 to your age, then print the numbers in descending order
            numbers[4] = 31;
            var descendingAge = numbers.OrderByDescending(x => x);
            foreach (var x in descendingAge) Console.WriteLine(x);
            Console.WriteLine("------------------------------------");
            ...
            var fullNames = employees.Where(x => x.FirstName.StartsWith("C") ||  x.FirstName.StartsWith("S")).OrderBy(x => x.FirstName);
            foreach (var n in fullNames) Console.WriteLine(n.FullName);
            Console.WriteLine("------------------------------------");
            //TODO: Print all the employees' FullName and Age who are over the age 26 to the console and order this by Age first and then by FirstName in the same result.
            var ageGroup = employees.Where(x => x.Age > 26).OrderBy(x => x.Age).ThenBy(x => x.FirstName);
            foreach (var e  in ageGroup) Console.WriteLine($"{e.FullName} {e.Age}");
